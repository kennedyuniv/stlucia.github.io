...
let students = {};
function loadStudents(htmlRowsDivId, b) {
    getStudents(b, function (res) {
        ...
        for (let index = 0; index < studentsArray.length; index++) {
            ...
            let studentDivBody = studentRowDivBody.replace("$PartnerCode$", student.centerCode)
                ...
                .replace("$Branch$", student.branch ? student.branch : "")
...
function getStudents(b, onSuccess, onError, hideAlert) {
    let branchQuery = (b==""||b==undefined) ? "" : "&branch=" + b;
    let url = getBaseURL() + "/students?limit=1000" + branchQuery;
    let headers = getAdminHeaders();
    callAPI(url, function (err, response) {
        if (err) {
            if (onError) {
